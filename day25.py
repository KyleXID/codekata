...
def solution(record):
  ...
  for i in range(len(record)):
    splitrecord.append(record[i].split())

  for i in range(len(splitrecord)):
    if splitrecord[i][0] == "Enter":
      newrecord[splitrecord[i][1]] = splitrecord[i][2]
    elif splitrecord[i][0] == "Change":
      newrecord[splitrecord[i][1]] = splitrecord[i][2]
    print(newrecord)

  for i in range(len(splitrecord)):
    if splitrecord[i][0] == "Enter":
      strrecord.append(f"{newrecord[splitrecord[i][1]]}님이 들어왔습니다.")
    if splitrecord[i][0] == "Leave":
      strrecord.append(f"{newrecord[splitrecord[i][1]]}님이 나갔습니다.")
...
def solution(record):
    ...
    for el in record:
        splitted = el.split(' ')
        if splitted[0] in ['Enter', 'Change']:
            nick[splitted[1]] = splitted[2]
        
    for el in record:
        splitted = el.split(' ')
        
        if splitted[0] != 'Change':
            answer.append(nick[splitted[1]] + sen[splitted[0]])
