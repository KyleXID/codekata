...
def solution(N, stages):
    ...
    for i in range(1, N+1):
        ...
        rate = (tryCount-success)/tryCount if tryCount else 0
        answer.append({ "rate": rate, "stage": i })

    answer = sorted(answer, key=lambda k: k['rate'], reverse=True)
    print(answer)

    return [d["stage"] for d in answer]
